...
function DashboardPreviewComponent() {
  ...
  const { ref, animate } = useRevealAnimation(defaultViewport);
    ...
    >
      ...
      >
        <motion.div
          ...
          className="relative w-full overflow-hidden rounded-[28px] border border-white/10 bg-[#FFFFFF] text-left shadow-[0_32px_64px_-12px_rgba(3,7,18,0.45)] will-change-transform"
        >
          <div className="flex items-center border-b border-white/5 bg-[#0B1528] px-5 py-3.5">
            ...
            <div className="mx-auto flex h-7 w-full max-w-sm items-center justify-center gap-2 rounded-md border border-white/5 bg-[#1E293B]/60 px-3 text-[12px] text-white/40">
              ...
              <span className="truncate font-light tracking-wide text-white/60">
          ...
          <div className="flex min-h-[360px] sm:min-h-[420px]">
            <aside className="hidden w-20 shrink-0 flex-col border-r border-[#F1F5F9] bg-[#FFFFFF] p-4 sm:flex md:w-56">
              <nav className="space-y-1">
                {[
                  ...
                  <div
                    key={item.label}
                    className={`flex cursor-pointer items-center gap-3 rounded-xl px-4 py-3 text-[13px] font-medium ${
                      item.active
            ...
            <main className="min-w-0 flex-1 bg-[#F8FAFC] p-4 sm:p-6 md:p-8">
              <div className="mb-6 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="mb-0.5 text-[12px] font-medium tracking-normal text-[#94A3B8]">
                    Good morning,
                  </p>
                  <h3 className="truncate text-lg font-bold tracking-tight text-[#1E293B] sm:text-xl">
                    Dr. Priya Sharma
                  </h3>
                </div>

                <div className="flex shrink-0 items-center gap-3">
                  <button className="flex h-9 w-9 items-center justify-center rounded-full bg-[#EEF2F6] text-[#64748B]">
              ...
              <div className="mb-6 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
                {[
                  ...
                  <div
                    key={kpi.label}
                    className="card-hover hover:border-primary/30 rounded-[16px] border border-[#E2E8F0] bg-white p-3 text-center sm:p-4"
                  >
                    <p
                      className={`text-[10px] font-semibold tracking-wider uppercase sm:text-[11px] ${kpi.labelColor} mb-2`}
                    >
                      {kpi.label}
                    </p>
                    <p
                      className={`text-xl font-extrabold tracking-tight sm:text-2xl md:text-3xl ${kpi.color}`}
                    >
                      {kpi.value}
                    </p>
                    <p className="mt-1 text-[9px] font-bold text-[#10B981] sm:text-[10px]">
                      {kpi.change}
                    </p>
                  </div>
                ))}
              ...
              <div className="rounded-[16px] border border-[#E2E8F0] bg-white p-4 md:p-6">
                ...
                <motion.div
                  className="flex h-20 items-end gap-1.5 px-1 sm:h-24 sm:gap-2 md:h-32 md:gap-3"
                  initial="hidden"
                  animate={animate}
                >
