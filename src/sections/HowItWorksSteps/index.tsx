        ...
        <StaggerContainer className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {HOW_IT_WORKS_FEATURES.map((item) => (
            <StaggerItem key={item.id}>
              <div
                className={`border-border h-full rounded-[var(--radius-card-lg)] border p-8 ${
                  isDark ? 'bg-surface-alt' : 'bg-white'
                }`}
              >
                ...
                <p className="font-display text-text mb-3 text-2xl font-medium">
                  {item.title}
                </p>
